...
def parse_data(data_dir):
        ...
        # dir_list = z.namelist()
    path_train = list_file_with_prefix(data_dir, "TRAIN")
    path_test = list_file_with_prefix(data_dir, "TEST")
    train_set = csv_to_X_y(path_train)
    test_set = csv_to_X_y(path_test)
...
def main():
    ...
    for data in data_dir_list:
        data_dir = 'data/test'
        set_seed()
        file_path = os.listdir(os.path.join(data_dir, data))
        full_paths_set = set()
        for file in file_path:
            path = data_dir + '/' + data + '/' + file
            full_paths_set.add(path)
        arg_dict = {
            "algorithm": args.alg,
        ...
        #loading the data
        data_dir = filedict['data_dir']
        ds = parse_data(full_paths_set)
        array_1 = ds[0]  
        list_100 = ds[1]  
        array_reshape = array_1.reshape(ds[0].shape[0], ds[0].shape[2])

        valid_rows = ~np.isnan(array_reshape).any(axis=1)

        filtered_array = array_reshape[valid_rows]
        filtered_list = [list_100[i] for i in range(len(list_100)) if valid_rows[i]]
        filtered_list = np.array(filtered_list)
        filtered_array_reshaped = filtered_array.reshape(-1, 1, ds[0].shape[2])
        ds_new = (filtered_array_reshaped, filtered_list)
        # Evaluation index
        metrics = [NMI(),RI()]
        algorithm.train(ds_new, valid_ds=None, valid_func=metrics)
