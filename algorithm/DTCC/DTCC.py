...
from algorithm.DTCC.lstm import LSTM
...
class DTCC(nn.Module):
    def __init__(self, configs):
        ...
        # Time Encoder
        self.time_encoder = LSTM(configs.input_size, configs.hidden_size, configs.num_layers, configs.output_size,
    ...
    def forward_cluster(self, x_in_t):
        ...
        f_time = self.conv_block1_f(x_in_t)
        f_time = self.conv_block2_f(f_time)
        f_time = self.conv_block3_f(f_time)
        f_time = f_time.reshape(x.shape[0], -1)
        z_time = self.cluster_projector_t(h_time)
        c = torch.argmax(z_time, dim=1)
        return c,h_time,f_time
